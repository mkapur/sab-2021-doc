...
Northwest Fisheries Science Center (NWFSC) West Coast Groundfish Bottom Trawl (WCGBT) Survey `r cpueyearrange["NWCBO"]`, and relative abundance indices and age data from the NWFSC slope survey `r cpueyearrange["NWSLP"]`,the Alaska Fisheries Science Center (AFSC) Slope Survey `r cpueyearrange["AKSLP"]`, and the Triennial Shelf Survey `r cpueyearrange["AKSHLF"]`. Input sample sizes were based on the number of tows length and marginal age compositions, whereas CAAL input sample sizes were based on the number of fish sampled. 
...
*  Reported commercial and reconstructed landings `r paste(range(model$catch$Yr), collapse = "-")`.

*  Biological data (ages) from the commercial port sampling programs
`r paste(range(c(model$len_comp_fit_table$Yr, model$age_comp_fit_table$Yr)), collapse = "-")`. Input sample sizes for the composition data were based on the number of port samples.
...
`r paste(range(model$discard[model$discard$Fleet == 1, "Yr"]), collapse = "-")` and 
`r paste(range(model$discard$Yr[model$discard$Yr < 2002]),collapse="-")`
...
\end{comment}
Conditional age-at-length (CAAL) data from the WCGBT Survey are used in the base model because these are the most representative source of `r spp` age and length data from the U.S. West Coast. 
...
Discard rates ranged from `r paste(with(subset(model$discard,Fleet == 1),round(range(Obs),2)),collapse = '-')` for the fixed gear fishery over the period `r paste(with(subset(model$discard,Fleet == 1),range(Yr)),collapse = '-')` (Figure \ref{discard_fit}). The early estimates of discard rates for the trawl fishery from the 1980s averaged `r round(mean(model$discard$Obs[model$discard$Yr < 1990 & model$discard$Fleet == 3]) * 100,2)`. More recent trawl estimates peaked in 2002 at `r round(mean(model$discard$Obs[model$discard$Yr == 2002 & model$discard$Fleet == 3]) * 100,2)`. After the implementation of the catch share program in 2011, discard rate estimates for the trawl fleet have dropped as low as `r round(mean(model$discard$Obs[model$discard$Yr == 2012 & model$discard$Fleet == 3]) * 100,2)` in 2012, with the highest recent observed rate of `r round(mean(model$discard$Obs[model$discard$Yr == 2019 & model$discard$Fleet == 3]) * 100,2)` in `r model$discard[model$discard$Yr > 2010,][which.max(model$discard$Obs[model$discard$Yr > 2010]),]$Yr`.
...
$ln(M)$ = `r paste(model$parameters[grep('NatM',model$parameters$Label),c('Prior','Pr_SD')][1,], collapse=",")`

` for females and
$ln(M)$ = `r paste(model$parameters[grep('NatM',model$parameters$Label),c('Prior','Pr_SD')][2,], collapse=",")`` for males 
...
The log of the unexploited recruitment level, $ln(R_0)$, for the Beverton-Holt stock-recruitment function was estimated, as were annual recruitment deviations beginning at the model start, `r model$startyr`. The main period of recruitment deviation estimation was chosen based on the first year of available sea-level data (i.e., 
...
The estimate of the target spawning biomass was `r derived_quants[derived_quants$Label == "SSB_Btgt", "Value"]`, ` r derived_quants[derived_quants$Label == "SSB_Btgt", "lci"]`-` r derived_quants[derived_quants$Label == "SSB_Btgt", "uci"]`, $\sim$95%  interval), which gives a catch of ` r derived_quants[derived_quants$Label == "Dead_Catch_Btgt", "Value"]` mt `r derived_quants[derived_quants$Label == "Dead_Catch_Btgt", "lci"]`-`r derived_quants[derived_quants$Label == "Dead_Catch_Btgt", "uci"]`, $\sim$95% interval). The stock was estimated to be just below the target stock size in the beginning of \Sexpr{myyear} at `r derived_quants[derived_quants$Label == paste0("SSB_", model$endyr), "Value"]` mt ` r derived_quants[derived_quants$Label == paste0("SSB_", model$endyr), "lci"]`-`r derived_quants[derived_quants$Label == paste0("SSB_", model$endyr), "uci"]`, $\sim$95% interval). 
